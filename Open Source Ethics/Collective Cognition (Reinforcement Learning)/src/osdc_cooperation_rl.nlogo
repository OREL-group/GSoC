...
globals [
  ...
  ;uncertainty
  ;reevaluate_rate
  churn_rate  ;; output of model
  repo_size
  ;unit
  ;horizon
  dynamics_of_coop  ;; graph
...
members-own [
  ...
  quan
  engag
  ;; qual quan code related. engag how much they talk
...
;    set reward-list []
  ...
  ask members [
    ...
    qlearningextension:state-def-extra ["qual" "quan"] [bla]
    (qlearningextension:actions [take-action])
    qlearningextension:reward [rewardFunc]
    qlearningextension:end-episode [isEndState] resetEpisode
    qlearningextension:action-selection "e-greedy" [0.5 0.08]
    qlearningextension:learning-rate 1
...
to resetEpisode
  ;setxy 0 0
    ;; resetting and updating world at the same time
